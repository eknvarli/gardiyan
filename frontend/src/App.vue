  ...
  <div class="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
    <div class="absolute inset-0 overflow-hidden">
      <div class="absolute -inset-10 opacity-20">
        ...
        <div class="absolute top-1/3 right-1/4 w-64 h-64 bg-purple-500 rounded-full mix-blend-multiply filter blur-xl animate-pulse animation-delay-2000"></div>
        <div class="absolute bottom-1/4 left-1/2 w-64 h-64 bg-cyan-500 rounded-full mix-blend-multiply filter blur-xl animate-pulse animation-delay-4000"></div>
      </div>
    </div>

    <div v-if="securityAlert.show" class="fixed inset-0 bg-red-900 bg-opacity-90 z-50 flex items-center justify-center">
      <div class="bg-white p-8 rounded-2xl shadow-2xl max-w-md mx-4">
        ...
        <button @click="dismissSecurityAlert" class="w-full bg-red-600 text-white py-3 rounded-lg font-semibold hover:bg-red-700 transition-colors">
          Tamam
        </button>
      </div>
    </div>

    <div v-if="sessionTimeoutWarning" class="fixed top-4 right-4 bg-orange-500 text-white p-4 rounded-lg shadow-lg z-40">
        ...
        <div class="max-w-md w-full">
          <div class="bg-white/10 backdrop-blur-lg rounded-3xl shadow-2xl border border-white/20 p-8">
            ...
            <form @submit.prevent="handleLogin" class="space-y-6">
              ...
              <div v-if="loginAttempts > 0" class="bg-red-500/20 border border-red-500/30 rounded-lg p-3">
              ...
              >
                ...
                <span v-else>Güvenli Giriş</span>
              </button>
            </form>
            ...
            <div class="overflow-x-auto">
              <table class="w-full">
                ...
                <tbody class="divide-y divide-white/10">
                  <tr v-for="license in filteredLicenses" :key="license.id" class="hover:bg-white/5 transition">
                    <td class="px-6 py-4">
                      <div class="flex items-center space-x-3">
                        <div class="w-2 h-2 bg-green-400 rounded-full" v-if="license.is_active"></div>
                        <div class="w-2 h-2 bg-red-400 rounded-full" v-else></div>
                        <code class="text-white font-mono text-sm bg-white/10 px-2 py-1 rounded">{{ license.key }}</code>
                        <button @click="copyToClipboard(license.key)" class="text-white/50 hover:text-white transition">
                    ...
                    <td class="px-6 py-4">
                      <span :class="license.is_active ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'" class="px-3 py-1 rounded-full text-xs font-medium">
                    ...
                    <td class="px-6 py-4 text-right">
                      <div class="flex items-center justify-end space-x-2">
                        <button 
                          ...
                          :class="license.is_active ? 'bg-orange-500 hover:bg-orange-600' : 'bg-green-500 hover:bg-green-600'"
    ...
    <div v-if="showAddModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div class="bg-slate-800 rounded-2xl shadow-2xl border border-white/20 max-w-md w-full">
        ...
        <form @submit.prevent="addLicense" class="p-6">
          <div class="mb-4">
...
export default {
  ...
  computed: {
    filteredLicenses() {
      ...
      if (this.searchQuery) {
        filtered = filtered.filter(license => 
          license.key.toLowerCase().includes(this.searchQuery.toLowerCase())
        )
      }
    ...
    userInitials() {
      return this.credentials.username ? this.credentials.username.charAt(0).toUpperCase() : 'A'
    }
  },
  async mounted() {
    this.detectIP()
  ...
  methods: {
    ...
    async handleLogin() {
      if (this.loginAttempts >= this.maxLoginAttempts) {
        this.showSecurityAlert('Güvenlik Kilidi', 'Çok fazla başarısız giriş denemesi. Lütfen 5 dakika bekleyin.')
        return
      }
      ...
      try {
        ...
        if (response.ok && data.token) {
          ...
          this.showSecurityAlert('Giriş Başarısız', data.error || 'Kullanıcı adı veya şifre hatalı.')
        }
      } catch (err) {
        this.loginAttempts++
        this.showSecurityAlert('Bağlantı Hatası', 'Sunucuya bağlanılamadı. Lütfen CORS ayarlarını kontrol edin.')
    ...
    resetSessionTimer() {
      ...
      this.lastActivity = new Date().toLocaleTimeString('tr-TR')
    ...
    async fetchLicenses() {
      ...
      try {
        const token = localStorage.getItem('authToken')
        const response = await fetch(`${this.API_BASE}/licenses/`, {
          headers: {
        ...
        if (response.ok) {
          ...
          this.handleLogout()
          this.showSecurityAlert('Oturum Süresi Doldu', 'Lütfen tekrar giriş yapın.')
        }
      } catch (err) {
        this.showSecurityAlert('Veri Hatası', 'Lisanslar yüklenirken hata oluştu.')
    ...
    calculateStats() {
      ...
      this.stats.active = this.licenses.filter(l => l.is_active).length
      this.stats.inactive = this.licenses.filter(l => !l.is_active).length
      const weekAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)
      this.stats.recent = this.licenses.filter(l => new Date(l.created_at) > weekAgo).length
    },
    async addLicense() {
      this.loading = true
      try {
        ...
        if (response.ok) {
          ...
          this.showSecurityAlert('Hata', error.detail || 'Lisans eklenirken hata oluştu.')
        }
      } catch (err) {
        this.showSecurityAlert('Bağlantı Hatası', 'Sunucuya bağlanılamadı.')
    ...
    async toggleLicense(license) {
      try {
        const token = localStorage.getItem('authToken')
        const response = await fetch(`${this.API_BASE}/licenses/${license.id}/`, {
          method: 'PATCH',
          headers: {
            'Authorization': `Token ${token}`,
            'Content-Type': 'application/json'
        ...
        if (response.ok) {
          ...
          this.showSecurityAlert('Hata', 'Lisans güncellenirken hata oluştu.')
        }
      } catch (err) {
        this.showSecurityAlert('Hata', 'Lisans güncellenirken hata oluştu.')
    ...
    async deleteLicense(licenseId) {
      ...
      try {
        const token = localStorage.getItem('authToken')
        const response = await fetch(`${this.API_BASE}/licenses/${licenseId}/`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Token ${token}`
          }
        ...
        if (response.ok) {
          ...
          this.showSecurityAlert('Hata', 'Lisans silinirken hata oluştu.')
        }
      } catch (err) {
        this.showSecurityAlert('Hata', 'Lisans silinirken hata oluştu.')
